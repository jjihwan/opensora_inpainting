...
class T2V_Inpainting_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.use_image_num != 0 and self.use_img_from_vid:
                ...
                pass
            mask = torch.ones_like(video) #(3, 17, 256, 256)
            mask[...,65:193,65:193] = 0
            masked_video = video * mask
            
            return video, mask, masked_video, input_ids, cond_mask
...
class T2V_Inpainting_inference_dataset(T2V_Inpainting_dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            return video, mask, masked_video
        except Exception as e:
            print(f'Error with {e}, {self.samples[idx]}')
            return self.__getitem__(random.randint(0, self.__len__() - 1))