...
class LatteT2VInpainting(LatteT2V):
    ...
    def forward(self, hidden_states: torch.Tensor, *args, mask = None, masked_x = None, **kwargs):
        ...
        hidden_states = einops.rearrange(hidden_states, 'b c f h w -> (b f) c h w').contiguous().to(self.dtype)
        hidden_states = self.proj_in(hidden_states)
        hidden_states = einops.rearrange(hidden_states, '(b f) c h w -> b c f h w', b=B).contiguous()
        print(f"before super {hidden_states.shape}")
