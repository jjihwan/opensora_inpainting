...
from opensora.dataset.transform import ToTensorVideo, TemporalRandomCrop, RandomHorizontalFlipVideo, CenterCropResizeVideo
...
class VideoInpaintingPipeline(DiffusionPipeline):
    r"""
    ...
    def get_video_from_path(self, video_path: str, ae, num_frames, sample_rate, text_encoder_name, max_image_size, cache_dir, device):
        ae_norm = {
        ...
        temporal_sample = TemporalRandomCrop(num_frames * sample_rate)

        video_decoder = DecordInit()
        decord_vr = video_decoder(video_path)
        ...
        video = video_data.permute(0, 3, 1, 2) # T C H W
        video = transform(video)
        video = video.transpose(0, 1) # C T H W
        video = video[None].to(device=device, dtype=torch.float16)

        H, W = video.shape[-2:]
        mask = torch.ones_like(video[:,[0]])
    ...
    def __call__(
        ...
        # 3.1 Get video
        mask, masked_video = self.get_video_from_path(video_path, ae, num_frames, sample_rate, text_encoder_name, max_image_size, cache_dir, device)

        # 4. Prepare timesteps
        self.scheduler.set_timesteps(num_inference_steps, device=device)
        timesteps = self.scheduler.timesteps
        # 5. Prepare latents.
        latent_channels = 4 # modified from self.transformer.config.in_channels, the latents are dim 4
        ...
        #encode the video 
        masked_video = self.vae.encode(masked_video.to(dtype=prompt_embeds.dtype, device=device))
        
        
        B, C, F, H, W = masked_video.shape
        mask = torch.nn.functional.interpolate(
            mask, size=(F, H, W)
        ).to(dtype=prompt_embeds.dtype, device=device)
        
        imageio.mimwrite("./debug/masked_video.mp4", masked_video[0].permute(1,2,3,0).to("cpu"), fps=10, quality=9)  # highest quality is 10, lowest is 0
        imageio.mimwrite("./debug/mask.mp4", mask[0].permute(1,2,3,0).to("cpu"), fps=10, quality=9)  # highest quality is 10, lowest is 0
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                # imageio.mimwrite(f"./debug/latents_{t}.mp4", latents[0].permute(1,2,3,0).to("cpu"), fps=10, quality=9)  # highest quality is 10, lowest is 0
                latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                masked_video_input = torch.cat([masked_video] * 2) if do_classifier_free_guidance else masked_video
                ...
                print(f"noise_pred{t} min: {noise_pred.min()}, latent{t} max: {noise_pred.max()}")
                # compute previous image: x_t -> x_t-1
                latents = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]
                # print(f"latent{t} min: {latents.min()}, latent{t} max: {latents.max()}")
        ...
        if not output_type == 'latents':
            imageio.mimwrite(f"./debug/final_latent.mp4", latents[0].permute(1,2,3,0).to("cpu"), fps=10, quality=9)
            video = self.decode_latents(latents)
            print(f"")
            imageio.mimwrite(f"./debug/output_video.mp4", video[0], fps=10, quality=9)
        else:
            video = latents
    ...
    def decode_latents(self, latents):
        #latents are of shape (B, C, T, H, W)
        print(f"latents shape: {latents.shape}")
        print(f"latent min: {latents.min()}, latent max: {latents.max()}")
        video = self.vae.decode(latents)
        print(f"video min: {video.min()}, video max: {video.max()}")
        print(f"video shape: {video.shape}") #(B, T, C, H, W)
        imageio.mimwrite(f"./debug/decoded_video.mp4", video[0].permute(0,2,3,1).to("cpu"), fps=10, quality=9)
