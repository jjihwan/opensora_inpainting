...
from pipeline_videoinpainting import VideoInpaintingPipeline
...
def main(args):
    ...
    vae = getae_wrapper(args.ae)(args.ae_path, subfolder="vae", cache_dir='cache_dir').to(device, dtype=torch.float16)
    if args.enable_tiling:
        vae.vae.enable_tiling()
        vae.vae.tile_overlap_factor = args.tile_overlap_factor

    # Load model:
    transformer_model = LatteT2VInpainting.from_pretrained(args.model_path, cache_dir="cache_dir", torch_dtype=torch.float16).to(device)
    ...
    videogen_pipeline = VideoInpaintingPipeline(vae=vae,
    ...
    if not isinstance(args.video_paths, list):
        args.video_paths = [args.video_paths]
    if len(args.video_paths) == 1 and args.video_paths[0].endswith('txt'):
        video_paths = open(args.video_paths[0], 'r').readlines()
        args.video_paths = [i.strip() for i in video_paths]

    for prompt, video_path in zip(args.text_prompt, args.video_paths):
        ...
        # inference_dataset = getdataset(args)
        # inference_dataloader = torch.utils.data.DataLoader(
        #     inference_dataset,
        #     # collate_fn=Collate(args),  # TODO: do not enable dynamic mask in this point
        #     batch_size=1
        # )
        # for (video, mask, masked_video) in inference_dataloader:
        #     video = video.to("cuda")
        #     mask = mask.to("cuda")
        #     masked_video = masked_video.to("cuda")
        #     print(f'video shape: {video.shape}, mask shape: {mask.shape}, masked_video shape: {masked_video.shape}')
        #     videos = videogen_pipeline(prompt,
        #                             video_length=video_length,
        #                             height=image_size,
        #                             width=image_size,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ae_path", type=str, default='LanguageBind/Open-Sora-Plan-v1.0.0')
    parser.add_argument("--sample_rate", type=int)
    parser.add_argument("--num_frames", type=int, default=17)
    parser.add_argument("--dataset", type=str, default='t2v_inpainting_inference')
