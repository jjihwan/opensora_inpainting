...
from opensora.models.diffusion.latte.modeling_latte import LatteT2VInpainting
from opensora.sample.pipeline_videoinpainting import VideoInpaintingPipeline
...
def main(args):
    ...
    model = Diffusion_models[args.model](
        in_channels=9, #modified from ae_channel_config[args.ae] = 4 to 9 to perform inpainting
    ...
    if args.pretrained:
        ...
        original_pos_embed_weights = torch.tensor(checkpoint['pos_embed.proj.weight']).clone()
        if original_pos_embed_weights.shape[1] == 4:  # If the original model has 4 channels
            new_pos_embed_weights = torch.zeros_like(model.pos_embed.proj.weight)  # Initialize new weights to zero
            new_pos_embed_weights[:, :4, :, :] = original_pos_embed_weights  # Copy original weights to the first 4 channels
            checkpoint['pos_embed.proj.weight'] = new_pos_embed_weights.clone()  # Replace the weights in the checkpoint
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, (x, input_ids, cond_mask) in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                x = x.to(accelerator.device)  # B C T+num_images H W
                ...
                # print(f"device:{torch.cuda.current_device()}, Before vae, Memory Allocated: {torch.cuda.memory_allocated() / 1e6} MB")
                ...
                with torch.no_grad():
                    ...
                    if args.use_image_num == 0:
                        # imageio.mimwrite(
                        # "./input.mp4", x[0].permute(1, 2, 3, 0).to("cpu"), fps=24, quality=9)  # highest quality is 10, lowest is 0
                        #No problem for loading the data, check ./input.mp4
                        x = ae.encode(x)  # B C T H W
                        # imageio.mimwrite(
                        # "./encoded_input.mp4", x[0].permute(1, 2, 3, 0).to("cpu"), fps=10, quality=9)  # highest quality is 10, lowest is 0
                        B, C, T, H, W = x.shape
                        mask = torch.ones(B, 1, T, H, W).to(x.device)
                        mask[:, :, :, H//4:H*3//4, W//4:W*3//4] = 0
                        masked_x = x * mask
                        # imageio.mimwrite(
                        # "./masked_x.mp4", masked_x[0].permute(1, 2, 3, 0).to("cpu"), fps=10, quality=9)  # highest quality is 10, lowest is 0
                ...
                # print(f"device:{torch.cuda.current_device()}, After vae, Memory Allocated: {torch.cuda.memory_allocated() / 1e6} MB")
                # print(f"Max Memory Allocated: {torch.cuda.max_memory_allocated() / 1e6} MB")
                # print()
                ...
                # print(f"device:{torch.cuda.current_device()}, After forward, Memory Allocated: {torch.cuda.memory_allocated() / 1e6} MB")
                # print(f"Max Memory Allocated: {torch.cuda.max_memory_allocated() / 1e6} MB")
                # print()
                ...
                # print(f"device:{torch.cuda.current_device()}, After backprop, Memory Allocated: {torch.cuda.memory_allocated() / 1e6} MB")
                # print(f"Max Memory Allocated: {torch.cuda.max_memory_allocated() / 1e6} MB")
                # print()
            ...
            if accelerator.is_main_process:
                validation_prompt = "a serene winter scene. Dominating the foreground is a vast expanse of snow-covered ground, its pristine white surface untouched and undisturbed. The snow extends as far as the eye can see, creating a sense of tranquility and isolation.  In the middle ground, a cluster of evergreen trees stands tall. Their dark green foliage provides a stark contrast to the surrounding snow, adding depth to the otherwise monochromatic landscape.  The background is shrouded in a thick fog, obscuring the distant trees and rolling hills beyond. This foggy veil adds an element of mystery to the scene, making the landscape appear both familiar and alien at the same time.  Above, the sky is a blanket of gray, its overcast nature suggesting that more snow might be on its way. The absence of any visible sunlight or shadows further enhances the cold, wintery mood of the scene.  Overall, the scene paints a picture of a quiet, snowy day in a forested area, where nature's quiet beauty is on full display. The precise locations of the objects and their relative positions contribute to the depth and perspective of the scene. The scene does not contain any discernible text or countable objects, and there are no visible actions taking place. The scene is a still life, capturing a single moment in time in this snowy forest."
                validation_video_path = "/workspace/Open-Sora-Plan/dataset/validation/mixkit-prairie-snow-in-a-pine-forest-3406_000.mp4"
                if global_step % args.checkpointing_steps == 0:
                    ...
                    if args.enable_tracker:
                        with torch.no_grad():
                            # create pipeline
                            ae_ = getae_wrapper(args.ae)(args.ae_path, cache_dir=args.cache_dir).to(accelerator.device, dtype=torch.float16)
                            if args.enable_tiling:
                                ae_.vae.enable_tiling()
                                ae_.vae.tile_overlap_factor = args.tile_overlap_factor
                            
                            ae_.latent_size = latent_size
                            ae_.eval()
                            # text_enc_ = get_text_enc(args).to(accelerator.device).eval()
                            model_ = LatteT2VInpainting.from_pretrained(save_path, subfolder="model", torch_dtype=torch.float16).to(accelerator.device).eval()
                            ...
                            text_enc_ = T5EncoderModel.from_pretrained(args.text_encoder_name, cache_dir=args.cache_dir, torch_dtype=torch.float16).to(accelerator.device).eval()
                            videos = []

                            with torch.autocast(device_type='cuda', dtype=weight_dtype):
                                if args.validation_sample_method == 'DDIM':  #########
                                    scheduler = DDIMScheduler()
                                elif args.validation_sample_method == 'EulerDiscrete':
                                    scheduler = EulerDiscreteScheduler()
                                elif args.validation_sample_method == 'DDPM':  #############
                                    scheduler = DDPMScheduler()
                                elif args.validation_sample_method == 'DPMSolverMultistep':
                                    scheduler = DPMSolverMultistepScheduler()
                                elif args.validation_sample_method == 'DPMSolverSinglestep':
                                    scheduler = DPMSolverSinglestepScheduler()
                                elif args.validation_sample_method == 'PNDM':
                                    scheduler = PNDMScheduler()
                                elif args.validation_sample_method == 'HeunDiscrete':  ########
                                    scheduler = HeunDiscreteScheduler()
                                elif args.validation_sample_method == 'EulerAncestralDiscrete':
                                    scheduler = EulerAncestralDiscreteScheduler()
                                elif args.validation_sample_method == 'DEISMultistep':
                                    scheduler = DEISMultistepScheduler()
                                elif args.validation_sample_method == 'KDPM2AncestralDiscrete':  #########
                                    scheduler = KDPM2AncestralDiscreteScheduler()
                            videogen_pipeline = VideoInpaintingPipeline(vae=ae_,
                                text_encoder=text_enc_,
                                tokenizer=tokenizer_,
                                scheduler=scheduler,
                                transformer=model_).to(device=accelerator.device)
                            
                            image_size = int(args.version.split('x')[1])
                            generated_video = videogen_pipeline(validation_prompt,
                                                        video_length=video_length,
                                                        height=image_size,
                                                        width=image_size,
                            ...
                            imageio.mimwrite(
                                f"./validation{global_step}.mp4", generated_video[0].to("cpu"), fps=10, quality=9)  # highest quality is 10, lowest is 0

                            # generated_video is shape of B, F, H, W, C (1, 17, 256, 256, 3)
                            #TODO: maybe saving the validation video would be a good option (not only sending it to wandb)
                            videos.append(generated_video.permute(0,1,4,2,3)) # B, F, C, H, W
                        ...
                        del ae_, text_enc_, model_, diffusion_, tokenizer_, videos, videogen_pipeline
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--validation_sample_method",
        type=str,
        default='DDIM',
        choices=['DDIM', 'EulerDiscrete', 'DDPM', 'DPMSolverMultistep', 'DPMSolverSinglestep', 'PNDM', 'HeunDiscrete', 'EulerAncestralDiscrete', 'DEISMultistep', 'KDPM2AncestralDiscrete'],
    )
    parser.add_argument(
        "--output_dir",
        type=str,
