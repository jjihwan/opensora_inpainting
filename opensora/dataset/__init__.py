...
from .inpainting_datasets import T2V_Inpainting_dataset, T2V_Inpainting_inference_dataset
...
def getdataset(args):
    ...
    if args.dataset == 'landscope':
    ...
    elif args.dataset == 't2v_inpainting':
        transform = transforms.Compose([
            ToTensorVideo(),
            CenterCropResizeVideo(args.max_image_size),
            RandomHorizontalFlipVideo(p=0.5),
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.text_encoder_name, cache_dir='./cache_dir')
        return T2V_Inpainting_dataset(args, transform=transform, temporal_sample=temporal_sample, tokenizer=tokenizer)
    elif args.dataset == 't2v_inpainting_inference':
        transform = transforms.Compose([
            ToTensorVideo(),
            CenterCropResizeVideo(args.max_image_size),
            RandomHorizontalFlipVideo(p=0.5),
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.text_encoder_name, cache_dir='./cache_dir')
        return T2V_Inpainting_inference_dataset(args, transform=transform, temporal_sample=temporal_sample, tokenizer=tokenizer)
    else:
