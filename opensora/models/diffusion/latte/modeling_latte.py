...
class LatteT2V(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        """
        # breakpoint()
        input_batch_size, c, frame, h, w = hidden_states.shape #(2,4,5,32,32)
        # print(hidden_states.shape, input_batch_size, c, frame, h, w, use_image_num)
        # print(timestep)
        # print(encoder_hidden_states.shape)
        # print(encoder_attention_mask.shape)
        frame = frame - use_image_num  # 20-4=16
        hidden_states = rearrange(hidden_states, 'b c f h w -> (b f) c h w').contiguous() #(10,4,32,32)
        # ensure attention_mask is a bias, and give it a singleton query_tokens dimension.
        #   we may have done this conversion already, e.g. if we came here via UNet2DConditionModel#forward.
        #   we can tell by counting dims; if ndim == 2: it's a mask rather than a bias.
        # expects mask of shape:
        ...
        if self.is_input_patches:  # here
            height, width = hidden_states.shape[-2] // self.patch_size, hidden_states.shape[-1] // self.patch_size
            num_patches = height * width #16*16=256
...
class LatteT2VInpainting(LatteT2V):
    @register_to_config
    def __init__(
            self,
            num_attention_heads: int = 16,
    ...
    def forward(self, hidden_states: torch.Tensor, *args, mask = None, masked_x = None, **kwargs):
        # print(f'hidden_states {hidden_states.shape}, mask {mask.shape}, masked_x {masked_x.shape}')
        hidden_states = torch.cat([hidden_states, mask, masked_x], dim=1)
...
def LatteT2VInpainting_XL_122(**kwargs):
    return LatteT2VInpainting(num_layers=28, attention_head_dim=72, num_attention_heads=16, patch_size_t=1, patch_size=2,
                    norm_type="ada_norm_single", caption_channels=4096, cross_attention_dim=1152, **kwargs)
...
Latte_models = {
    ...
    "LatteT2VInpainting-XL/122": LatteT2VInpainting_XL_122,
}
